...
Проект покрыт двумя группами тестов:

**Интеграционные тесты API** 

(`CarsControllerTests`) поднимают приложение целиком через `WebApplicationFactory<Program>` и обращаются к нему по HTTP, как настоящий клиент. Чтобы тестам не требовался запущенный PostgreSQL, `CustomWebApplicationFactory` подменяет провайдер базы на EF Core In-Memory. Проверяется, что `GET /api/cars` возвращает `200 OK`, а `POST /api/cars` создаёт машину и возвращает `201 Created`.

**Тесты репозиториев консольной версии** 

(`CarRepositoryTests`) создают временную базу SQLite и проверяют репозитории: добавление и чтение машин, удаление по id, проверку существования.
...
5. Swagger UI
6. Валидация ввода (DataAnnotations) и корректные коды ответов (400/404)
7. Интеграционные тесты API через WebApplicationFactory
8. Релиз v2.0.0

**В планах:**

9. Возможный вынос общего слоя данных в отдельную библиотеку
